...
from amlink import module_controller, socket_server, config, utils
...
class Network:
    ...
    def toEngine(self, id, cid, username, command, arguments, socketio):
        ...
        if command not in self.local_commands:
            ...
            self.toClient(socket_client.Response.initFromResp(result))
    ...
    def toClient(self, resp):
        ...
        if message == 'success':
            ...
            if linkProcess and 'intent' in data.keys and 'ner' in data.keys:
                intent = data['intent']
                ner = socket_client.parse_ner(data['ner'][0], data['ner'][1])
